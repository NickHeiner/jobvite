...
function getData() {
    if (!options.jobvite_username || !options.jobvite_password || !options.report_id) {
        ...
        qFail('options is missing required fields');
    } else {
...
function getLogin() {
    ...
    return qNotify('Getting login').then(q.nfcall(Request, settings)).then(fp.tail(login));
}
