...
function getData() {
    if (!options.jobvite_username || !options.jobvite_password || !options.report_id) {
        ...
        return qFail('options is missing required fields');
    } else {
        return qNotify('Data validated').then(getLogin);
    }
}
...
function getLogin() {
    ...
    return qNotify('Getting login')
        .then(function(notified) {
            console.log('notified', notified);
        })
        .then(q.nfcall(Request, settings).then(login));
}

function login(response, body) {
    console.log('login', arguments.length);
    ...
    return qNotify('Got login')
        .then(qNotify('Get next login'))
        .then(q.nfcall(Request.post, settings))
        .spread(getReport);
}

function getReport(response, body) {
    ...
    return qNotify('Generating report').then(q.nfcall(Request, settings)).then(fp.tail(generateReport));
}


function generateReport(response, body, callback) {
    var cookies = response.headers['set-cookie'];
    var viewstate = Hacks.getViewstate(body);
    ...
    return qNotify('Processing data').then(q.nfcall(Request, settings)).then(fp.tail(process));
}
