...
function getLogin() {
    ...
    return qNotify('Getting login')
        .then(q.nfcall(Request, settings).spread(login));
}
...
function getReport(response, body) {
    ...
    return qNotify('Generating report').then(q.nfcall(Request, settings)).spread(generateReport);
}

function generateReport(response, body) {
    var cookies = response.headers['set-cookie'];
    var viewstate = Hacks.getViewstate(body);
    ...
    return qNotify('Processing data').then(q.nfcall(Request, settings)).then(process);
}
